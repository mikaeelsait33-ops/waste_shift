...
test('manager can navigate the complete restaurant workspace', async ({ page, isMobile }) => {
  ...
  await navigation.getByRole('button', { name: /Stock|Inventory/ }).click();
  await expect(page.getByRole('heading', { name: 'Invoices, Ingredients & Stock' })).toBeVisible();
  const invoiceViews = page.locator('[aria-label="Invoice views"]');
  await expect(invoiceViews.getByRole('button', { name: 'Upload & Review' })).toBeVisible();

  await invoiceViews.getByRole('button', { name: 'Raw ingredients' }).click();
  await expect(page.getByRole('heading', { name: 'Ingredient Library' })).toBeVisible();

  await invoiceViews.getByRole('button', { name: 'Processed invoices' }).click();
  ...
  await expect(page.getByRole('heading', { name: 'Recipes & Ingredients' })).toBeVisible();
...
test('manager can log waste and see it in the shared log', async ({ page, isMobile }) => {
  ...
  const wasteLogButton = isMobile
    ? navigation.locator('.bottom-nav-button').filter({ hasText: 'Waste' })
    : navigation.locator('.nav-button').filter({ hasText: 'Waste Log' });
